...
description: 将网站转换为带讲解音频和字幕的视频。每张幻灯片自动生成旁白文案，支持 TTS 语音合成，视频中同步显示字幕和播放讲解音频。
---

# Website to Video with Script

将任意网站转换为带讲解音频和字幕的专业视频。每张幻灯片自动生成文案，支持 TTS 语音合成，视频中同步显示字幕和播放讲解音频。
...
# 指定 TTS 语音
/website-to-video-with-script https://example.com --tts-voice zh-CN-XiaoyiNeural
...
# 只生成音频，不渲染视频
/website-to-video-with-script https://example.com --audio-only
```
...
| 语音 ID | 名称 | 语言 | 性别 |
|---------|------|------|------|
| `zh-CN-XiaoyiNeural` | 晓艺 | 中文 | 女声（默认） |
| `zh-CN-YunxiNeural` | 云希 | 中文 | 男声 |
| `zh-CN-XiaoxiaoNeural` | 晓晓 | 中文 | 女声 |
| `en-US-JennyNeural` | Jenny | 英文 | 女声 |
...
async def generate_tts(text, output_file, voice="zh-CN-XiaoyiNeural"):
...
| `--tts-voice` | TTS 语音 | zh-CN-XiaoyiNeural |
...
A: 支持！默认使用 `edge-tts` 生成中文女声讲解音频。可通过 `--tts-voice` 参数切换语音，或使用 `--no-audio` 禁用。
...
### v1.1.0 (2026-05-23)
- 新增 TTS 讲解音频功能（edge-tts）
- 支持多种中文/英文语音
- 音频与幻灯片精确同步
- 新增 `--no-audio`、`--tts-voice`、`--tts-speed` 参数
- 新增 `--audio-only` 参数
